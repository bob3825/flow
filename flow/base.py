...
class Once(object):
    ...
    Inherit this if you want to bypass parallelization and not run as a daemon.
    """
    pass
...
class EventBased(object):
    ...
    Inherit this if you want Flow to run your source's run method once, which
    initiates some kind of event listener. Upon event the source should call
    its callback, which will be spawning the start method on the worker as a
    new thread.
    ...
    should do so for any asynchronous operations.
    """
